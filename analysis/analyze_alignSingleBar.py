...
usage = "usage: python analysis/analyze_alignArray.py -i /media/cmsdaq/ext/data/ALIGNMENT/ALIGNBAR_14_02_2020 -o /media/cmsdaq/ext/data/ALIGNMENT/ALIGNBAR_14_02_2020"
...
for file in list_allfiles:
    ...
    if ("_coincidences.root" in file):
        ...
        for event in range (0,tree.GetEntries()):
            ...
            if(tree.energy[1]>-9 and tree.energy[2]>-9):
        ...
        #scan Y
        if (POS >= 0 and POS <= 20):
            histoY.SetBinContent(histoY.GetXaxis().FindBin(Y),nhits_coinc)
            histoY.SetBinError(histoY.GetXaxis().FindBin(Y),sqrt(nhits_coinc))
...
histoY.GetYaxis().SetTitle("Number of pixel+bar coincidence hits")
